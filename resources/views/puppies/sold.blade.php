...
@section('title')
    <h1>{!! __('puppies.titles.sold_puppies') !!}</h1>
@stop
...
@section('content')
    ...
    @foreach($puppies as $puppy)
        ...
        <div class="col-md-12">
            <p><strong>{!! __('puppies.fields.seller') !!}:</strong> {{ $puppy->employee_surname }} {{ $puppy->employee_name }}
                ({{ $puppy->employee_login }})</p>
        </div>
        <div class="col-md-12">
            <p><strong>{!! __('puppies.fields.customer') !!}:</strong> {{ $puppy->user_surname }} {{ $puppy->user_name }} ({{ $puppy->user_login }}
                )</p>
        </div>
        <div class="col-md-12">
            <p><strong>{!! __('puppies.fields.date_time') !!}:</strong> {{ $puppy->date }}</p>
        </div>
        @if (Auth::check())
