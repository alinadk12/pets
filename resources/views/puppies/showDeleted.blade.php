...
@section('title')
    <h1>{!! __('puppies.titles.deleted_puppies') !!}</h1>
@stop
        ...
        @if (Auth::check())
            @if (Auth::user()->hasRole('Администратор'))
                <div class="text-right">
                    <a class="button" href="/puppies/restore/{{$puppy->id}}">{!! __('puppies.buttons.restore') !!}</a>
                    <a class="button" href="/puppies/forcedelete/{{$puppy->id}}">{!! __('puppies.buttons.remove_db') !!}</a>
                </div>
            @endif
        @endif
        <br>
